...
max_steps = 5000            # train step upbound
batch_size = 100            
...
def RNN(X, weights, biases):
    # X ==> (100 batches * 100 steps, 24 inputs)
    X = tf.reshape(X, [-1, n_inputs])

    # X_in = W*X + b
    X_in = tf.matmul(X, weights['in']) + biases['in']
    # X_in ==> (100 batches, 100 steps, 30 hidden)
...
with tf.Session() as sess:
    ...
    # restore the model saved last time
    ...
    #data_x, data_y = help.generateData(preload=False, win_size=0.6, method="3d")
    ...
    if sys.argv[1] == "-train":
        while step < max_steps:
            ...
            # every 20 steps make a test and save the current model
            if step % 20 == 0:
